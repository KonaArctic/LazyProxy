...
func main() {
	...
	lzypxy.MyAddr, err = netip.ParsePrefix(os.Args[1])
	if err != nil || !lzypxy.MyAddr.Addr().Is4() || lzypxy.MyAddr.Bits() >= 30 {
		os.Exit(2)
	}
	...
	flags.Func("resolv", "", func(value string) error {
		...
		lzypxy.Resolv[value[0:strings.LastIndex(value, "=")]], err = netip.ParseAddr(value[strings.LastIndex(value, "=")+1 : len(value)])
		if err != nil {
			return errors.New("invalid argument")
		}
		return nil
	...
	var errchn chan error = make(chan error, 3)
	go func() {
		errchn <- lzypxy.ServeDNS()
	}()
	go func() {
		errchn <- lzypxy.ServeDHCP()
	}()
	if <-errchn != nil {
		os.Exit(1)
	}
